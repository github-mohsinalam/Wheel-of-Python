...
class WOFHumanPlayer(WOFPlayer):
    ...
    def getMove(self, obscuredPhrse, guessed):
        prompt = """{} has ${}
        ...
        userInput = input(prompt.format(self.name, self.prizeMoney, category, obscuredPhrase, ','.join(guessed)) )
        return userInput 
...
class WOFComputerPlayer(WOFPlayer):
    ...
    def getMove(self, category, obscuredPhrase, guessed):
        poss_guess = self.getPossibleLetters(guessed)
        if len(poss_guess) == 0 :
            return 'pass'
        if self.smartCoinFlip():
            i = len(self.SORTED_FREQUENCIES) - 1
            while i > 0 :
                if (self.SORTED_FREQUENCIES)[i] in poss_guess :
                    return (self.SORTED_FREQUENCIES)[i]
                i = i - 1
        return random.choice(poss_guess)
